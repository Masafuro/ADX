...
> [!IMPORTANT]
> ### 💻 開発用 PC 接続時の原則必須要件：【ガルバニック絶縁型】RS-485 変換アダプターの使用
> 本ボードへのファームウェア書き込みやデバッグ等で開発 PC と接続する場合、**「ガルバニック絶縁型（Galvanically Isolated）の RS-485 - USB シリアル変換アダプター」の利用を原則必須**とします。
> 
> 絶縁型アダプターを使用することで、PC の USB GND と本ボードの共通 GND が電気的に完全遮断（絶縁耐圧 1kV〜3kV）されます。これにより、万一電源逆接続などで基板の GND プレーンが +12V に浮上した場合でも、**最も高価で被害の大きい「開発 PC（USB ポートやマザーボード）の焼損・道連れ破壊」を 100% 回避** できます。
> 
> （※基板1枚ごとに高価なデジタルアイソレータや絶縁 DC-DC を載せずに $5〜7 の低コスト化を達成する代わりに、**「1個数千円の絶縁アダプターを開発治具として導入し、全ノードで使い回す」** という極めて合理的かつ安全な運用方針です。）
...
### 3. ガルバニック絶縁型変換器による PC 保護メカニズム
```mermaid
graph TD
    subgraph SAFE["【防護シナリオ】絶縁アダプターによる短絡ループの物理的遮断"]
        PWR_REV["外部電源 (12V等)<br/>【逆接続ミス】<br/>(+)線 ↔ (-)線 が逆"]
        BOARD_GND["ADX CORE-S-P<br/>共通 GND プレーン<br/>(+12V 電位に浮上)"]
        COMM_CABLE["RS-485 ケーブル<br/>(A, B, GND)"]
        ISO_BARRIER["【ガルバニック絶縁バリア】<br/>磁気/光アイソレータ + 絶縁DC-DC<br/>(耐圧 1.5kV〜3kV)"]
        PC_USB["開発用 PC / USB ポート<br/>正常な 0V GND"]

        PWR_REV -- "(+)電圧が印加" --> BOARD_GND
        BOARD_GND --> COMM_CABLE
        COMM_CABLE -- "浮上電位が到達" --> ISO_BARRIER
        ISO_BARRIER -.- "❌ 短絡経路が完全遮断！<br/>(電流ループが閉じない)" -.- PC_USB
    end

    style SAFE fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px;
    style ISO_BARRIER fill:#c8e6c9,stroke:#1b5e20,stroke-width:2px;
    style PC_USB fill:#e3f2fd,stroke:#1565c0,stroke-width:2px;
```
開発 PC と本ボードの間に絶縁型アダプターを介在させることで、電気的なループが完全に断ち切られます。CORE-S-P 側の GND が +12V に浮上しても、PC 側へ電流が流れる経路（帰還路）が存在しないため、**PC 側の USB ポートやマザーボードは 100% 安全に保たれます。**

### 4. 絶縁アダプター使用時でも注意すべき境界条件
PC 側の破壊は絶縁アダプターで完全に防ぐことができますが、以下のケースでは依然として短絡ループが形成されるため注意してください：

1. **オシロスコープ・ロジックアナライザのプローブ GND**:
   - 一般的な卓上オシロスコープのプローブ GND クリップは、商用 AC コンセントの大地接地（FG）に直結しています。
   - 絶縁 USB アダプターを使っていても、**オシロの GND クリップを基板の GND に接続した瞬間に商用接地経由の短絡ループが完成**し、プローブ線の焼損やオシロスコープの入力部破損に至る恐れがあります（※バッテリー駆動のフローティングオシロや差動プローブの使用を推奨）。
2. **マルチノード（複数機器）接続時のバス**:
   - RS-485 バス上に複数の CORE-S-P や別電源の機器（モータードライバ、PLC 等）が接続されている場合、1台の逆接によって**他のノード基板が連鎖破壊されるリスク**は残ります（PC のみ助かり、バス上のノード同士が道連れになります）。
3. **IDC 2×10P 拡張コネクタへの外部電源接続機器**:
   - 拡張コネクタの GND も同一プレーンです。別電源から給電されているセンサーやアクチュエータ基板と GND を接続している場合、そこがバイパス経路になります。

---

### 5. 推奨される絶縁型 USB-RS485 変換アダプターの仕様
本ボードの開発・書き込み用として、以下の仕様を満たす変換アダプターの使用を強く推奨します：

- **絶縁方式**: ガルバニック絶縁（磁気結合デジタルアイソレータまたは高速フォトカプラ）
- **絶縁耐圧**: 1,000 Vrms (1.5 kV DC) 以上
- **電源供給**: 内蔵絶縁 DC-DC コンバータ（PC の USB バスパワーから RS-485 側電源を絶縁生成するもの）
- **端子仕様**: `A+`, `B-`, `GND` 端子（3線接続）を備えていること
- **市販製品例**:
  - DSD TECH SH-U11G（産業用アイソレーション USB to RS485）
  - Waveshare USB TO RS485 (B)（絶縁型 RS485 変換モジュール）
  - その他 FTDI / Silicon Labs チップ＋ADuM / ISO シリーズアイソレータ搭載品
...
- **製造データ一式**:
  - 回路図・ネットリスト・製造データ: [`review/rev3/data/`](review/rev3/data/)
  - 発注直前チェックリスト（配置・極性確認）: [`review/rev3/component_placements_checklist.md`](review/rev3/component_placements_checklist.md)
