...
# ADX CORE-S (CORE-STANDARD)

**ADX CORE-S** は、次世代オープンハードウェアエコシステム「ADX」における **標準・開発者向けプロトタイピングマイコンボード（Standard / Hacker & Evaluation Edition）** です。
...
本基板は **100% OSHW（オープンソースハードウェア）** として回路図、PCB レイアウト、BOM、およびファームウェアを完全公開しており、開発者が JLCPCB 等を利用して 1 枚（小ロット）から安価に自作・改良することを前提に設計されています。
...
graph TD
    subgraph DANGER["【重大事故シナリオ】GND ループによるバイパス短絡経路"]
        ...
        BOARD_GND["ADX CORE-S<br/>共通 GND プレーン<br/>(+12V 電位に浮上！)"]
...
CORE-S は非絶縁構造ですが、**「基板単体での通電時」および「下流回路の異常時」** において最大限の安全性を確保するため、rev3 において保護回路が強化されています。
...
- **OTW (Over-The-Wire) ソフトウェア開発**: RS-485 経由での自律ブートローダー、BMC 連携電源制御、アップローダーツールの実機デバッグ。
  - *※CORE-S で開発したファームウェア・ブートローダー・アップローダーは、ADX 標準のピン配置および制御論理に基づくため、将来の派生ボードや上位機種にもそのまま継承可能です。*
...
- **製造コストの目安**:
  - 5pcs 発注時: 約 $7.6 / 台（セットアップ費用込み）
  - 30pcs 以上発注時: 約 $5.0 / 台 前後
