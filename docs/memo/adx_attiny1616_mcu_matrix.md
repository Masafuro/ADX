...
物理ピン配置やリボンケーブルのシールド構造については、[ADX マイコンボード ピンアウト仕様書 (v2)](./adx_pinout_v2.md) を参照してください。本ドキュメントでは、外部拡張用の 2×10ピン IDCコネクタ（基板回路図上のリファレンス例: CN2）の各端子番号を **「IDC Pin 1〜20」** と表記します。
