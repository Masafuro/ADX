...
### 3.4 送信時名目ボーレート強制リセット仕様（正帰還ドリフト完全防止）

自律分散型ピンポンリレーにおいて、LINAUTO による自動同期を安全かつ長期的に成立させるため、本仕様書は以下のレジスタ制御を**必須仕様（MUST）**として規定します。

```text
【正帰還累積ドリフトの防止構造】
[Node 1] ───(自機名目 BAUD で送信)───> [Node 2: LINAUTO で受信 (BAUD が一時校正)]
                                              │
                                              ▼ ★送信直前に自機名目 BAUD へ強制リセット！
[Node 1: LINAUTO で受信 (BAUD が一時校正)] <───(自機名目 BAUD で返信)─── [Node 2]
      │
      ▼ ★送信直前に自機名目 BAUD へ強制リセット！
```

1. **正帰還累積ドリフトの原理とリスク:**
   * マイコンの `USART.BAUD` レジスタは送信クロック発生器と受信サンプリング回路で共有されています。
   * LINAUTO 受信によって書き換わった動的 `BAUD` のまま次フレームを送信すると、相手ノードがその値からさらに微小な測定誤差を含んで再校正を行い、**双方向で誤差が増幅し続ける「正帰還累積ドリフト」** が発生します。
   * これを放置すると、数千周期でボーレートが許容帯域を逸脱し、バトンドロップ（通信破綻）に至ります。
2. **必須規定（Mandatory Requirement）:**
   * **「全ノードは、Break 送出（フレーム送信開始）の直前に、必ず自機内部発振器基準の名目ボーレート（`NOMINAL_BAUD`）を `USART.BAUD` レジスタへ強制再代入（リセット）しなければならない。」**
3. **達成される効果:**
   * **送信は常に自機基準（9600 bps）、相手フレームへの追従は受信時のみに限定** されます。
   * 正帰還ループが物理的に完全遮断され、数千万周期以上の連続稼働においてもサイクルタイム $T_{\text{cycle}}$ のドリフトがゼロとなり、極小ジッター（実測 $\pm 31\,\mu\text{s}$）での長期連続周回が保証されます。
